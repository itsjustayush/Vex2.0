(() => {
  ...
  const typingExercises = [
    ...
    { id:"alice-rabbit-hole", level:"Novel", title:"Down the rabbit-hole", text:"Alice was beginning to get very tired of sitting by her sister on the bank, and of having nothing to do: once or twice she had peeped into the book her sister was reading, but it had no pictures or conversations in it, and what is the use of a book without pictures or conversations?", book:"Alice’s Adventures in Wonderland", author:"Lewis Carroll", source:"https://www.gutenberg.org/files/11/11-h/11-h.htm" },
    { id:"secret-garden", level:"Novel", title:"A new beginning", text:"When Mary Lennox was sent to Misselthwaite Manor to live with her uncle everybody said she was the most disagreeable-looking child ever seen. It was true, too. She had a little thin face and a little thin body, thin light hair and a sour expression.", book:"The Secret Garden", author:"Frances Hodgson Burnett", source:"https://www.gutenberg.org/files/113/113-h/113-h.htm" },
    { id:"sherlock-woman", level:"Novel", title:"The woman", text:"To Sherlock Holmes she is always the woman. I have seldom heard him mention her under any other name. In his eyes she eclipses and predominates the whole of her sex.", book:"The Adventures of Sherlock Holmes", author:"Arthur Conan Doyle", source:"https://www.gutenberg.org/files/1661/1661-h/1661-h.htm" }
  ];
...
<label class="primary-btn">${icon("plus")} Add media<input type="file" accept="image/*,video/*" multiple hidden data-file-upload /></label><button class="pill-btn" data-action="add-note">${icon("note")} <span>Note</span></button></div></div><div class="page-meta"><span>Endless canvas · drag to pan · scroll to zoom</span><div class="page-switcher">${state.boards.map(item => `<button class="${item.id === state.activeBoardId ? "active" : ""}" data-action="select-board" data-board-id="${item.id}">${escapeHtml(item.title)}</button>`).join("")}<button data-action="zoom-board" data-zoom="out">−</button><button class="active" data-zoom-label>${zoomPercent}%</button><button data-action="zoom-board" data-zoom="in">+</button><button data-action="zoom-board" data-zoom="reset">reset</button></div></div><div class="moodboard" data-moodboard><div class="mood-canvas" style="transform:translate(${state.boardPan.x}px,${state.boardPan.y}px) scale(${state.boardZoom})">${state.mood.map(renderMoodItem).join("")}</div>${state.mood.length === 0 ? `<div class="mood-empty"><div><strong>Your canvas is wide open.</strong>Drop in an image, video, or note to begin.</div></div>` : ""}</div>${renderMoodInspector()}</section>`;
  ...
  function renderLanding() {
    document.getElementById("app").innerHTML = `<div class="landing"><div class="landing-shell">${renderTopbar("landing")}<main><section class="landing-hero"><span class="eyebrow"><b>✦</b> writing, notes & moodboards</span><h1>Make room for <em>good</em> thoughts.</h1><p class="landing-subtitle">A calm, colorful workspace for the thoughts that refuse to sit still. Write in full flow, pin the fragments, and let the connections appear.</p><div class="landing-cta"><button class="primary-btn" data-action="open-app">Start writing ${icon("arrow")}</button><button class="ghost-btn" data-action="scroll-demo">See the workspace <span>↓</span></button></div></section><section class="demo-wrap" id="demo"><div class="demo-frame"><div id="demo-host"></div></div></section><section class="landing-bento"><article class="feature-card"><div class="feature-icon">✺</div><h3>Thoughts, not folders.</h3><p>Start with a blank page, choose a texture, and make your own little corner of the internet.</p></article><article class="feature-card"><div class="feature-icon">⌘</div><h3>Markdown native.</h3><p>Formatting, LaTeX, shortcuts, and a gentle keyboard that makes writing feel tactile.</p></article><article class="feature-card"><div class="feature-icon">◌</div><h3>Endless moodboards.</h3><p>Drop in images, videos, notes, and references without fighting the canvas.</p></article><article class="feature-card"><div class="feature-icon">≈</div><h3>Three moods.</h3><p>Light, dark, and zen. The room changes when you do.</p></article><article class="feature-card"><div class="feature-icon">↗</div><h3>Write freely.</h3><p>Explore as a guest in memory, then sign in when you are ready to save and sync your space.</p></article></section><section class="landing-aeo-section"><div class="landing-section-head"><span class="eyebrow"><b>✦</b> architecture &amp; capabilities</span><h2>How does Vex elevate personal writing and creative thought?</h2><p>Vex is designed for thinkers, writers, and builders who need both the precision of structured Markdown notes and the spatial freedom of infinite moodboards.</p></div><div class="faq-grid"><article class="faq-card"><h3>How does the tactile keyboard sound simulation work?</h3><p class="aeo-faq-answer">Keystrokes trigger calibrated mechanical sound sprites through the browser Web Audio API, synthesizing the tactile acoustic resonance of physical typewriter switches. The sound can be dynamically adjusted or muted via the audio control toggle.</p></article><article class="faq-card"><h3>How do endless moodboards organize visual concepts?</h3><p class="aeo-faq-answer">The infinite 2D canvas lets creators drag images, videos, and color-coded sticky notes into freeform clusters. Users can zoom from 25% to 200% and pan smoothly to discover organic associations between fragments of thought.</p></article><article class="faq-card"><h3>How does Markdown and LaTeX formatting work in Vex?</h3><p class="aeo-faq-answer">Vex supports standard Markdown syntax for headings, blockquotes, bullet lists, bold, italics, and inline code. Mathematical expressions enclosed in dollar signs (such as $E = mc^2$) render instantly with crisp typographic formatting.</p></article><article class="faq-card"><h3>How does cross-device cloud synchronization maintain privacy?</h3><p class="aeo-faq-answer">You can write immediately in guest mode without creating an account; your work remains strictly in local client memory. When you sign in via Google or email verification, documents sync securely to Google Cloud Firestore with automatic version history.</p></article><article class="faq-card"><h3>What display themes and paper textures are available?</h3><p class="aeo-faq-answer">Vex features three curated themes: Dark, Light, and Zen. The writing canvas can also be styled with five paper styles: ruled single, ruled double, plain, dotted light, and dotted dense to suit every writing state of mind.</p></article><article class="faq-card"><h3>How does the built-in typing practice mode work?</h3><p class="aeo-faq-answer">Enhance Typing tracks your real-time words-per-minute speed, accuracy, and typing cadence over curated literary excerpts, developing keyboard fluency and tactile muscle memory.</p></article></div><div class="specs-table-wrap"><table class="specs-table" aria-label="Vex Workspace Specifications"><thead><tr><th>Core Feature</th><th>Supported Capabilities &amp; Formats</th><th>Execution Architecture</th></tr></thead><tbody><tr><td><strong>Markdown &amp; LaTeX</strong></td><td>Headings (#), Quotes (&gt;), Lists (-), Math ($...$)</td><td>Client-side reactive parser</td></tr><tr><td><strong>Tactile Typing</strong></td><td>Mechanical switch sound sprites, mute toggle</td><td>Web Audio API synthesize engine</td></tr><tr><td><strong>Infinite Moodboard</strong></td><td>Sticky notes, media drop, pan &amp; zoom (25%-200%)</td><td>Spatial 2D coordinate canvas</td></tr><tr><td><strong>Cloud Sync</strong></td><td>Google Cloud Firestore, multi-device, version history</td><td>Encrypted batch cloud sync</td></tr><tr><td><strong>AI Intelligence</strong></td><td>Gemini 3.8 Flash note assistance &amp; summarization</td><td>Server-side API proxy</td></tr></tbody></table></div><nav class="internal-nav-strip" aria-label="Explore Vex Documentation and System Status"><a href="/docs">✦ Documentation &amp; User Guide</a><a href="/status">● System Status &amp; Health</a><a href="/dashboard">↗ Open Workspace</a><a href="/llms.txt">🗎 llms.txt Manifest</a></nav></section></main><footer class="landing-footer"><span>© 2026 Vex. Think in full color.</span><div class="landing-footer-links"><a href="/docs">Docs</a><a href="/status">Status</a><a href="/llms.txt">llms.txt</a><a href="https://github.com/itsjustayush/Vex2.0" target="_blank" rel="noreferrer">GitHub</a></div><span class="creator-credit">Created by <a href="https://github.com/itsjustayush" target="_blank" rel="noreferrer">Ayush Bhattacharya</a> · <a href="mailto:info.cometlabs@gmail.com">info.cometlabs@gmail.com</a></span><span class="inspiration-credit">Inspired by <a href="https://keythm.aayushbharti.in/" target="_blank" rel="noreferrer">keythm.aayushbharti.in</a></span><a class="github-badge" href="https://github.com/itsjustayush/Vex2.0" target="_blank" rel="noreferrer" aria-label="View Vex on GitHub"><span aria-hidden="true">◉</span> Vex on GitHub ↗</a></footer></div></div>`;
