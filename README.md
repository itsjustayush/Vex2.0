...
The client detects the existing Firebase configuration for authentication and media storage. Durable notes, moodboards, board items, preferences, and typing statistics are stored in the Vex Supabase project through direct Supabase REST requests carrying the current Firebase ID token, with a Firebase-verified Flask bridge as a fallback. Every path is scoped by the Firebase Auth UID. The original Firebase email-link helper remains preserved for backwards compatibility with previously issued links, but new email sign-ins use the server-side Resend OTP flow.
...
The durable Supabase schema is rooted at the Firebase Auth UID: `vex_pages` stores note documents, `vex_boards` stores board metadata, `vex_board_items` stores individual moodboard pieces, `vex_settings` stores preferences, and `vex_typing_stats` stores per-user typing statistics. Firebase-scoped RLS policies accept only tokens from the configured Firebase project whose subject matches the row’s `user_id`. Firebase Storage remains responsible for moodboard media under `users/{uid}/moodboard/{fileName}`. The browser uses the publishable Supabase key only; the optional `SUPABASE_SERVICE_ROLE_KEY` is restricted to the Flask fallback bridge and must never be exposed to browser code.

## Deployment persistence variables

In addition to the Firebase and Resend variables, production uses `SUPABASE_URL=https://qdsdjgfvimuvdujxouab.supabase.co` and the publishable key `SUPABASE_PUBLISHABLE_KEY`. The optional server-only `SUPABASE_SERVICE_ROLE_KEY` enables the Flask fallback bridge when direct Supabase access is unavailable. Cross-device sync is available only after the user signs in or signs up, which keeps every saved workspace tied to a verified Firebase Auth UID.
